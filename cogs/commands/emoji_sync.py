...
class EmojiSync(commands.Cog):
    ...
    async def _collect_assets(self) -> list[EmojiAsset]:
        # Local assets win so redesigned packs replace legacy CDN downloads.
        raw_assets = await self._collect_from_asset_dirs()
        raw_assets.extend(await self._collect_from_source_guilds())
        raw_assets.extend(await self._collect_from_code_emojis())
